...
torch::Tensor w4a16_forward_direct(
    ...
    int group_size)
{
    ...
    TORCH_CHECK(activations.dim() >= 2,
                "activations must have shape [..., IF]");

    int64_t OF_packed = W_packed.size(0);
    int64_t IF = W_packed.size(1);
    int64_t OF = OF_packed * 4;
    TORCH_CHECK(activations.size(-1) == IF,
                "activations.shape[-1] must equal IF");

    std::vector<int64_t> output_shape;
    output_shape.reserve(activations.dim());
    int64_t B = 1;
    for (int64_t i = 0; i < activations.dim() - 1; ++i) {
        output_shape.push_back(activations.size(i));
        B *= activations.size(i);
    }
    output_shape.push_back(OF);
    ...
    auto activations_flat = activations.view({B, IF});
    auto activations_colmajor = activations_flat.transpose(0, 1).contiguous();
    ...
    w4a16_gemv_vectorized_kernel<<<blocks, threads>>>(
        ...
        reinterpret_cast<__nv_bfloat16*>(activations_colmajor.data_ptr<at::BFloat16>()),
    ...
    return OUT.transpose(0, 1).contiguous().view(output_shape);
}

PYBIND11_MODULE(w4a16_cuda_direct_ext_v7, m) {
    m.def("forward", &w4a16_forward_direct, "W4A16 GEMV forward pass (CUDA, BF16, direct activation layout)");
}
